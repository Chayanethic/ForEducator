  ...
  return (
    <>
      ...
      <header className="bg-white shadow-sm p-4 md:p-5 flex justify-between items-center z-10 sticky top-0 border-b border-slate-200">
        <div>
          <h1 className="text-lg md:text-xl font-black text-slate-800">Welcome, {user?.firstName || "Guest"}!</h1>
        ...
        <div className="flex items-center gap-3">
           {activeRoadmap?.streak !== undefined && (
             <div id="tour-streak" className="bg-rose-50 border border-rose-100 px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-2 cursor-default">
      ...
      <div className="p-4 md:p-5 lg:p-6 max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6">
          ...
          <div className="lg:col-span-8 space-y-5 lg:space-y-6">
            ...
            <div id="tour-join-room" className="bg-gradient-to-br from-indigo-600 to-purple-700 p-6 rounded-3xl shadow-lg relative overflow-hidden flex flex-col lg:flex-row lg:items-center justify-between gap-6 group">
              ...
              <div className="relative z-10 flex-1">
                ...
                <p className="text-indigo-200 text-xs md:text-sm font-medium max-w-md">Enter the secure Room ID provided by your educator to instantly access your scheduled live exam.</p>
              </div>
              
              {/* ⚡ RESPONSIVE FORM FIX ⚡ */}
              <form onSubmit={handleJoinRoom} className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto relative z-10 shrink-0">
                <input 
                  type="text" 
                  value={joinCode} 
                  onChange={(e) => setJoinCode(e.target.value)} 
                  placeholder="e.g. 8xV9-2mB" 
                  className="w-full sm:w-48 md:w-56 bg-white/10 border border-white/20 rounded-xl p-3 md:p-3.5 text-white placeholder-indigo-200 outline-none focus:bg-white/20 focus:border-white/40 transition font-mono text-sm font-bold shadow-inner" 
                  required 
                />
                <div className="w-full sm:w-auto shrink-0">
                  <GuestBlocker role="student">
                    <button type="submit" disabled={isJoining} className="bg-white text-indigo-700 px-6 py-3 md:py-3.5 rounded-xl font-black hover:bg-indigo-50 transition shadow-lg disabled:opacity-70 flex items-center justify-center gap-2 w-full whitespace-nowrap">
                      {isJoining ? <i className="fas fa-spinner fa-spin"></i> : "Join"}
                    </button>
                  </GuestBlocker>
                </div>
              </form>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {activeProgress.map((prog) => (
                    <GuestBlocker role="student" key={prog.id}>
                      <div onClick={() => router.push(`/student/exam/${prog.mockId}`)} className="bg-white p-4 rounded-xl border border-amber-200 shadow-sm relative overflow-hidden group hover:-translate-y-1 hover:shadow-md hover:border-amber-400 transition-all duration-200 cursor-pointer h-full flex flex-col">
                        ...
                        <h3 className="font-bold text-slate-900 mt-1 mb-4 truncate text-sm flex-1">{prog.mockTitle || "Live Mock Exam"}</h3>
                        <button className="w-full bg-slate-900 text-white py-2 rounded-lg text-xs font-black group-hover:bg-indigo-600 transition-colors shadow-sm flex items-center justify-center gap-1.5 mt-auto">Resume Exam <i className="fas fa-play text-[10px]"></i></button>
                      </div>
                    </GuestBlocker>
                  ))}
                </div>
                  ...
                  {publicMocks.map((mock) => (
                    <GuestBlocker role="student" key={mock.id}>
                      <div onClick={() => router.push(`/student/exam/${mock.id}`)} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-indigo-400 hover:-translate-y-1 hover:shadow-md transition-all duration-200 flex flex-col h-full relative overflow-hidden group cursor-pointer w-full">
                        ...
                        <button className="w-full bg-indigo-50 text-indigo-700 border border-indigo-200 py-2 rounded-lg text-xs font-black group-hover:bg-indigo-600 group-hover:text-white transition-colors shadow-sm mt-auto">Start Mock Test</button>
                      </div>
                    </GuestBlocker>
                  ))}
                </div>
