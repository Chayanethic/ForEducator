    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 font-sans">
        <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md">
          ...
          <p className="text-center text-slate-500 mb-8 text-sm">Restricted access portal for PYQ Management.</p>
          <form onSubmit={handleLogin} className="space-y-4">
            {loginError && <div className="bg-rose-50 text-rose-600 p-3 rounded-lg text-sm font-bold text-center border border-rose-200">{loginError}</div>}
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Admin ID</label>
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 outline-none focus:border-rose-500 mt-1" required />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Passcode</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 outline-none focus:border-rose-500 mt-1" required />
            </div>
            <button type="submit" className="w-full bg-rose-600 text-white font-bold py-3 rounded-lg hover:bg-rose-700 transition mt-4 shadow-lg shadow-rose-500/30">Access Portal</button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="flex h-screen bg-slate-50 font-sans relative selection:bg-rose-100 selection:text-rose-900">
      ...
      {publishedRoomId && (
        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-6">
          <div className="bg-white p-10 rounded-3xl shadow-2xl text-center max-w-lg w-full border border-slate-100">
            ...
            <h2 className="text-3xl font-bold text-slate-800 mb-2">PYQ Published!</h2>
            <p className="text-slate-500 mb-8">
              This Official PYQ is now live on the platform under the <strong className="text-slate-700">{examCategory}</strong> category.
            </p>
            <button 
              onClick={() => {
      ...
      <aside className="w-64 bg-slate-950 text-white flex flex-col shrink-0 border-r border-slate-800">
        ...
        <nav className="flex-1 p-4 space-y-2">
            <button onClick={() => setActiveTab("create")} className={`w-full flex items-center gap-3 p-3 rounded-lg font-medium transition ${activeTab === 'create' ? 'bg-rose-600 text-white shadow-md shadow-rose-600/20' : 'text-slate-400 hover:bg-slate-900 hover:text-rose-400'}`}>
                <i className="fas fa-upload w-5"></i> Upload PYQ
            </button>
            <button onClick={() => setActiveTab("manage")} className={`w-full flex items-center gap-3 p-3 rounded-lg font-medium transition ${activeTab === 'manage' ? 'bg-rose-600 text-white shadow-md shadow-rose-600/20' : 'text-slate-400 hover:bg-slate-900 hover:text-rose-400'}`}>
                <i className="fas fa-database w-5"></i> Manage Mocks
            </button>
        </nav>
        <div className="p-4 border-t border-slate-800">
            <button onClick={() => setIsAuthenticated(false)} className="w-full flex items-center gap-3 text-slate-400 hover:text-white p-3 rounded-lg transition">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        {activeTab === "create" && (
          <>
            <header className="bg-white shadow-sm p-6 flex justify-between items-center z-10 sticky top-0">
              <div>
                <h1 className="text-2xl font-bold text-slate-800">PYQ Generation Studio</h1>
                <input 
                    type="text" value={examTitle} onChange={(e) => setExamTitle(e.target.value)}
                    className="text-sm font-bold text-rose-600 bg-transparent border-b border-slate-200 outline-none focus:border-rose-500 mt-2 pb-1 w-80"
                    placeholder="e.g. GATE CS 2021 Official"
                />
              </div>
              <button onClick={saveToDatabase} disabled={questions.length === 0 || isPublishing} className={`px-6 py-2.5 rounded-xl font-bold shadow-sm transition flex items-center gap-2 ${questions.length > 0 && !isPublishing ? 'bg-rose-600 text-white hover:bg-rose-700 shadow-rose-500/20' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
            ...
            <div className="p-6 md:p-8 space-y-8 max-w-5xl mx-auto w-full">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap items-center gap-6">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Exam Category</label>
                  <select value={examCategory} onChange={(e) => setExamCategory(e.target.value)} className="border border-slate-200 rounded p-1.5 text-sm font-bold outline-none focus:border-rose-500 bg-slate-50 text-rose-700 cursor-pointer">
                ...
                <div className="border-l border-slate-200 pl-6">
                  <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Duration (Mins)</label>
                  <input type="number" value={duration} onChange={(e)=>setDuration(e.target.value)} className="w-24 border border-slate-200 rounded p-1.5 text-sm font-bold outline-none focus:border-rose-500 bg-slate-50"/>
                </div>
                <div className="border-l border-slate-200 pl-6">
                  <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Tools</label>
                  <div className="flex items-center gap-2 mt-2">
                    <input type="checkbox" checked={allowCalculator} onChange={(e)=>setAllowCalculator(e.target.checked)} className="w-4 h-4 text-rose-600 cursor-pointer"/>
                    <span className="text-sm font-medium text-slate-700">Calc</span>
                  </div>
                </div>
                <div className="border-l border-slate-200 pl-6 flex-1">
                  <label className="text-xs font-bold text-rose-500 uppercase block mb-1">Live Feed Visibility</label>
                  <div className="flex items-center gap-2 mt-2">
                    <input type="checkbox" checked={showInLiveFeed} onChange={(e)=>setShowInLiveFeed(e.target.checked)} className="w-4 h-4 text-rose-600 cursor-pointer"/>
                    <span className="text-sm font-medium text-slate-700">Push to Student Dashboard 'Live' section</span>
                  </div>
                </div>
              </div>
              ...
              {questions.length === 0 && (
                <section className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                  <label className="border-2 border-dashed border-rose-200 rounded-xl bg-rose-50/30 hover:bg-rose-50 transition p-12 flex flex-col items-center justify-center text-center cursor-pointer group">
                    ...
                    <h3 className="text-xl font-bold text-slate-800 mb-2">{file ? file.name : "Upload Official PYQ PDF"}</h3>
                    <p className="text-sm text-slate-500 mb-6">Gemini will auto-extract images, text, and options.</p>
                    <button onClick={handleExtract} disabled={isProcessing || !file} className="bg-slate-900 text-white px-8 py-3 rounded-lg font-bold hover:bg-slate-800 transition disabled:bg-slate-400">{isProcessing ? "Extracting..." : "Commence Extraction"}</button>
                  </label>
                </section>
              )}
              ...
              {questions.length > 0 && (
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
                    <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide"><i className="fas fa-layer-group text-indigo-500 mr-2"></i> Section Architecture</h3>
                  ...
                  <div className="space-y-3">
                    {examSections.map((sec, i) => (
                      <div key={i} className="flex flex-wrap sm:flex-nowrap items-center gap-3 bg-slate-50 p-3 rounded-lg border border-slate-200">
                        <div className="w-8 h-8 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center font-bold text-xs shrink-0">{i+1}</div>
                        <input type="text" value={sec.name} onChange={(e) => updateSection(i, 'name', e.target.value)} placeholder="Section Name (e.g. General Aptitude)" className="flex-1 bg-white border border-slate-300 rounded-lg p-2 text-sm font-bold text-slate-800 outline-none focus:border-rose-500 shadow-sm min-w-[200px]"/>
                        <div className="flex items-center gap-2 bg-white border border-slate-300 rounded-lg p-1 pr-3 shadow-sm">
                          <input type="number" value={sec.count} onChange={(e) => updateSection(i, 'count', parseInt(e.target.value) || 0)} className="w-16 bg-transparent p-1 text-center text-sm font-bold text-indigo-600 outline-none"/>
                  ...
                  <button onClick={addSection} className="mt-4 text-xs font-bold text-indigo-600 bg-indigo-50 px-4 py-2 rounded-lg border border-indigo-100 hover:bg-indigo-100 transition shadow-sm"><i className="fas fa-plus mr-1"></i> Add Another Section</button>
                </div>
              )}
              ...
              {questions.length > 0 && (
                <div className="space-y-6">
                  {questions.map((q, qIndex) => (
                    <div key={qIndex} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm relative hover:border-rose-300 transition group">
                      ...
                      <div className="flex flex-wrap gap-3 mb-4 items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                        ...
                        <span className="bg-indigo-100 text-indigo-800 text-xs font-bold px-3 py-1.5 rounded-full border border-indigo-200 shadow-sm truncate max-w-[150px]">
                      ...
                      <textarea value={q.text} onChange={(e) => updateQuestionField(qIndex, 'text', e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-lg p-3 text-sm text-slate-900 mb-4 focus:ring-2 focus:ring-rose-500 outline-none resize-y font-bold shadow-inner" rows="3"/>
        ...
        {activeTab === "manage" && (
          <div className="p-6 md:p-8 max-w-5xl mx-auto w-full">
            <h1 className="text-2xl font-bold text-slate-800 mb-6">Manage Official PYQs</h1>
            
            {isLoadingMocks ? (
              <div className="text-center p-10"><i className="fas fa-spinner fa-spin text-3xl text-rose-500"></i></div>
            ) : adminMocks.length === 0 ? (
              <div className="bg-white p-10 rounded-2xl border border-slate-200 text-center shadow-sm"><i className="fas fa-folder-open text-4xl text-slate-300 mb-3"></i><p className="text-slate-500 font-medium">No Official PYQs published yet.</p></div>
            ) : (
                    ...
                    {adminMocks.map(mock => (
                      <tr key={mock.id} className="hover:bg-slate-50 transition">
                        <td className="p-4 pl-6 font-bold text-slate-800">
                          <span className="bg-rose-100 text-rose-700 text-[10px] uppercase px-2 py-0.5 rounded mr-2">Official</span>
                          {mock.title}
                          {mock.showInLiveFeed && <span className="ml-2 bg-indigo-100 text-indigo-700 text-[10px] uppercase px-2 py-0.5 rounded"><i className="fas fa-satellite-dish mr-1"></i> Feed</span>}
                        </td>
                        <td className="p-4 text-center font-medium text-slate-600">{mock.examCategory || "General"}</td>
                        <td className="p-4 text-center text-sm text-slate-500">{mock.createdAtDate?.toLocaleDateString()}</td>
                        <td className="p-4 text-right pr-6">
                          <button onClick={() => handleDeleteMock(mock.id)} className="text-rose-500 hover:text-rose-700 hover:bg-rose-50 p-2 rounded transition" title="Delete PYQ"><i className="fas fa-trash-alt"></i></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
