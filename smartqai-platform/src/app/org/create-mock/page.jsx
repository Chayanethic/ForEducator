...
export default function OrgCreateMockPage() {
  ...
  useEffect(() => {
    if (userLoaded && user) {
      const savedDraft = localStorage.getItem(`ozone_org_draft_${user.id}`);
      if (savedDraft) {
        try {
          ...
          if (parsed.questions?.length > 0) {
            ...
            showToast("Organization Draft Restored!", "success");
          }
  ...
  const handleBackNavigation = () => {
    if (questions.length > 0) setShowDraftModal(true);
    else router.push('/org/dashboard');
  };

  const saveDraftAndLeave = () => {
    const draftData = { 
      examTitle, examCategory, duration, questions, 
      blockMobile, blockMultiple, blockTabSwitch, enableWatermark, spotlightMode 
    };
    localStorage.setItem(`ozone_org_draft_${user.id}`, JSON.stringify(draftData));
    router.push('/org/dashboard');
  };

  const discardDraftAndLeave = () => {
    localStorage.removeItem(`ozone_org_draft_${user.id}`);
    setQuestions([]);
    router.push('/org/dashboard');
  };
  ...
  if (!userLoaded || !orgLoaded || !organization) return (
    <div className="flex h-screen items-center justify-center bg-slate-50 flex-col animate-in fade-in duration-500">
      <div className="w-24 h-24 bg-indigo-900 text-white rounded-[2rem] flex items-center justify-center text-4xl transform -rotate-3 animate-pulse shadow-xl shadow-indigo-900/40"><i className="fas fa-building"></i></div>
      <h2 className="text-xl font-black text-slate-900 mt-6 tracking-tight animate-pulse">Enterprise Workspace...</h2>
    </div>
  );
    ...
    <div className="flex h-screen bg-slate-100 font-sans relative overflow-hidden">
      ...
      {showSettingsModal && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[99999] flex justify-center items-start md:items-center pt-10 pb-10 md:pt-0 p-4 animate-in fade-in overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-2xl w-[95%] shadow-2xl border border-slate-200 my-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               ...
               <div className="space-y-5">
                 ...
                 <div>
                   ...
                   <input type="text" value={examCategory} onChange={(e) => setExamCategory(e.target.value)} placeholder="e.g. Weekly Test, GATE ECE" className="w-full bg-slate-50 border border-slate-300 rounded-xl p-3 text-sm font-bold text-slate-900 outline-none focus:border-indigo-500" />
                 </div>
        ...
        <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-md z-[99999] flex flex-col items-center justify-center p-6 animate-in fade-in zoom-in-95">
          <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-2xl w-full overflow-hidden border border-slate-200">
            ...
            <div className="bg-indigo-900 p-8 text-center text-white relative overflow-hidden">
              ...
              <div className="w-16 h-16 bg-emerald-400 text-white rounded-full flex items-center justify-center text-3xl mx-auto mb-4 shadow-lg relative z-10 border-4 border-white/20"><i className="fas fa-check"></i></div>
              <h2 className="text-3xl font-black relative z-10 tracking-tight">Enterprise Exam Ready!</h2>
              <p className="text-indigo-200 mt-2 font-medium relative z-10 text-sm">Your white-label exam is deployed on OZONE secure clusters.</p>
            </div>

            <div className="p-8">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-3 flex items-center gap-2">
                <i className="fas fa-code text-indigo-500"></i> White-Label Embed Code
              </h3>
              ...
              <div className="relative group">
                <pre className="bg-slate-900 text-emerald-400 p-5 rounded-xl text-[10px] md:text-xs font-mono overflow-x-auto whitespace-pre-wrap shadow-inner border border-slate-800">
{`<iframe 
  src="${baseUrl}/embed/exam/${publishedMockId}" 
...
></iframe>`}
                ...
                <button onClick={copyIframeCode} className={`absolute top-4 right-4 px-4 py-2 rounded-lg text-xs font-bold shadow-md transition-all ${copiedIframe ? 'bg-emerald-500 text-white' : 'bg-white text-slate-800 hover:bg-indigo-50'}`}>
                  {copiedIframe ? "Copied!" : "Copy HTML"}
                </button>
              </div>

              <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-slate-100">
                <button onClick={() => window.open(`${baseUrl}/embed/exam/${publishedMockId}`, '_blank')} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-800 py-3 rounded-xl font-bold transition flex justify-center items-center gap-2 text-sm">
                  <i className="fas fa-external-link-alt"></i> Preview Exam
                </button>
                <button onClick={() => router.push('/org/dashboard')} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold shadow-md transition text-sm">
                  Back to Portal
                </button>
              </div>
            </div>
      ...
      {/* --- SIDEBAR --- */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white flex flex-col shrink-0 transition-all border-r border-slate-800 transform md:relative md:translate-x-0 ${isMobileMenuOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
        <div className="h-16 flex items-center justify-between md:justify-start px-5 md:px-6 border-b border-slate-800">
          <div className="flex items-center gap-2">
            <img src={organization.imageUrl} alt="Org" className="w-8 h-8 rounded-md bg-white p-0.5" />
            <span className="font-black tracking-tight text-sm truncate">{organization.name}</span>
          </div>
        ...
        <nav className="flex-1 p-3 space-y-2 mt-4">
            <button onClick={() => router.push('/org/dashboard')} className="w-full flex items-center justify-start gap-3 text-slate-400 hover:bg-slate-800 p-3 rounded-xl transition">
                <i className="fas fa-home text-lg"></i> <span className="font-bold text-sm">Dashboard</span>
            </button>
            <button className="w-full flex items-center justify-start gap-3 bg-indigo-600 text-white p-3 rounded-xl shadow-md border-l-4 border-emerald-400">
                <i className="fas fa-file-pdf text-indigo-200 text-lg"></i> <span className="font-bold text-sm">Enterprise Studio</span>
            </button>
        </nav>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 h-auto md:h-16 py-3 px-4 md:px-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0 z-20 shrink-0 shadow-sm">
          <div className="flex items-center gap-3 w-full md:max-w-[60%]">
             ...
             <button onClick={handleBackNavigation} className="shrink-0 bg-slate-100 border border-slate-200 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-200 transition-all flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-xs md:text-sm font-bold shadow-sm">
               <i className="fas fa-arrow-left"></i> <span className="hidden sm:block">Back</span>
             </button>
             <input type="text" value={examTitle} onChange={(e) => setExamTitle(e.target.value)} className="text-base md:text-lg font-black text-slate-900 bg-transparent border-b-2 border-transparent hover:border-slate-200 focus:border-indigo-500 outline-none w-full max-w-[200px] sm:max-w-sm transition-colors truncate" placeholder="Exam Title" />
          </div>
          <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto justify-end shrink-0">
            ...
            <button onClick={confirmDeploy} disabled={questions.length === 0 || uploadingCount > 0 || isPublishing} className="flex-1 md:flex-none justify-center bg-indigo-900 text-white px-4 py-2 md:px-6 md:py-2 rounded-lg text-xs md:text-sm font-bold shadow-md hover:bg-indigo-800 transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
              {isPublishing ? "Deploying..." : "Publish & Embed"} <i className="fas fa-code"></i>
            </button>
          </div>
        </header>
            ...
            <div className="bg-white p-3 md:p-4 border-b border-slate-200 shrink-0 shadow-sm flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between z-10">
              ...
              <div className="flex items-center gap-3 w-full xl:w-auto justify-between xl:justify-end">
                 <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-lg border border-slate-200">
                   <input type="number" min="1" value={startPage} onChange={e => setStartPage(e.target.value)} className="w-8 md:w-10 text-center text-xs font-bold text-slate-900 bg-white border border-slate-300 rounded py-1 outline-none"/>
                   <span className="text-[9px] text-slate-400 font-black uppercase">To</span>
                   <input type="number" min="1" value={endPage} onChange={e => setEndPage(e.target.value)} className="w-8 md:w-10 text-center text-xs font-bold text-slate-900 bg-white border border-slate-300 rounded py-1 outline-none"/>
                 </div>
              ...
              <div className="max-w-4xl mx-auto space-y-4 pb-32">
                        ...
                        {isExpanded && (
                          <div className="p-4 md:p-5 bg-white">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 pb-4 border-b border-slate-100 gap-4">
                               <div className="flex flex-wrap gap-4 w-full sm:w-auto">
                                  ...
                                  <div className="flex items-center gap-2 sm:border-l border-slate-200 sm:pl-4">
                                    ...
                                    <input type="number" step="0.5" value={q.marks} onChange={(e) => updateQuestionField(qIndex, 'marks', e.target.value)} className="w-14 bg-white border border-slate-300 rounded text-xs px-2 py-1 text-center font-black outline-none focus:border-emerald-500 shadow-sm"/>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-black text-rose-700 uppercase">- Mk:</span>
                                    <input type="number" step="0.1" value={q.negativeMarks} onChange={(e) => updateQuestionField(qIndex, 'negativeMarks', e.target.value)} className="w-14 bg-white border border-slate-300 rounded text-xs px-2 py-1 text-center font-black outline-none focus:border-rose-500 shadow-sm"/>
                                  </div>
                               </div>
                            ...
                            <div className="relative mb-6 mt-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Enterprise Question Editor</label>
                              <textarea 
                              ...
                              />
                              <button onClick={() => toggleDictation(qIndex, 'text')} className={`absolute bottom-3 right-3 p-2 rounded-lg transition-colors shadow-sm border ${listeningField === `q-${qIndex}-text` ? 'bg-rose-100 text-rose-600 border-rose-300 animate-pulse' : 'bg-white text-slate-500 border-slate-200 hover:bg-indigo-50 hover:text-indigo-600'}`}>
                                      ...
                                      <div key={optIndex} className={`flex items-start gap-3 p-3 rounded-xl border transition-colors ${isCorrect ? 'border-emerald-500 bg-emerald-50 ring-1 ring-emerald-300' : 'border-slate-300 bg-slate-50 hover:border-slate-400'}`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                          <div className="relative mb-2">
                                            ...
                                            <button onClick={() => toggleDictation(qIndex, 'option', optIndex)} className={`absolute top-1.5 right-1.5 p-1.5 rounded transition border ${listeningField === `q-${qIndex}-opt-${optIndex}` ? 'bg-rose-100 text-rose-600 border-rose-200 animate-pulse' : 'bg-slate-100 text-slate-400 border-transparent hover:text-indigo-600 hover:bg-white'}`}>
                                              <i className="fas fa-microphone"></i>
                                            </button>
                                          </div>
                ...
                {questions.length > 0 && (
                  <div className="pt-6 pb-12 flex justify-center animate-in fade-in">
                    <button onClick={handleAddCustomQuestion} disabled={isProcessing} className="bg-white border-2 border-dashed border-emerald-300 text-emerald-600 px-8 py-4 rounded-2xl font-black hover:bg-emerald-50 hover:border-emerald-500 hover:-translate-y-1 transition-all shadow-sm flex items-center gap-3 group w-full max-w-md justify-center disabled:opacity-50">
