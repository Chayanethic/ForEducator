...
export default function AnalyticsDashboard({ params }) {
  ...
  // 2. Real-time Listener for Submissions & Leads
  useEffect(() => {
    if (!mockId || !organization) return;

    // Sort by highest score first by default
    const q = query(
  ...
  // 3. Export to CSV (Crucial B2B Feature)
  const exportToCSV = () => {
    ...
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 h-20 px-8 flex justify-between items-center z-10 shrink-0 shadow-sm">
          <div>
            <h1 className="text-2xl font-black text-slate-900">{examData?.title || "Loading..."}</h1>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {submissions.map((sub, index) => {
                        ...
                        // ⚡ THE 3-TIER BADGE LOGIC ⚡
                        let cheatBadge;
                        
                        if (sub.forcedSubmit || warningsCount >= 3) {
                          // TIER 3: Terminated (Red)
                          cheatBadge = (
                            <span className="inline-flex items-center gap-1.5 bg-rose-100 text-rose-800 border border-rose-300 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider shadow-sm">
                              <i className="fas fa-ban"></i> Terminated
                            </span>
                          );
                        } else if (warningsCount > 0) {
                          // TIER 2: 1 or 2 Warnings (Amber/Yellow)
                          cheatBadge = (
                            <span className="inline-flex items-center gap-1.5 bg-amber-100 text-amber-800 border border-amber-300 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider shadow-sm">
                              <i className="fas fa-exclamation-triangle"></i> {warningsCount}/3 Warnings
                            </span>
                          );
                        } else {
                          // TIER 1: Clean (Green)
