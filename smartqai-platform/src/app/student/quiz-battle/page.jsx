...
function QuizBattleContent() {
  ...
  useEffect(() => {
    if (!roomCode || (view === "home" || view === "create" || view === "join" && !roomData) || !user) return;
  ...
  }, [isRoundOver, isHost, roomData, roomCode, user]);
  ...
  const handleCreateRoom = async (e) => {
    ...
    // ⚡ GUEST MODE SIMULATION: Prepares a fake room lobby! ⚡
    if (!user) {
      setTimeout(() => {
        setRoomCode("DEMO99");
        setRoomData({
  ...
  const handleJoinRoom = async (e) => {
    e.preventDefault();
    if (!roomCode || roomCode.length < 6 || !user) return;
    setIsGenerating(true);
  ...
  const submitAnswer = async (index) => {
    if (hasSubmitted || !roomData || !user) return;
  ...
  const handleSearchSong = async (e) => {
    e.preventDefault();
    if (!songSearch.trim() || !user) return;
  ...
  return (
    // ⚡ Removed outer sidebars and absolute positioning to flow seamlessly inside layout.jsx ⚡
    <div className="flex flex-col min-h-full bg-slate-50 font-sans relative overflow-hidden selection:bg-indigo-100 selection:text-indigo-900">
        ...
        {view === "create" && (
          <div className="max-w-xl mx-auto w-full bg-white/90 backdrop-blur-xl p-8 md:p-12 rounded-[2rem] border border-white shadow-2xl animate-in slide-in-from-bottom-8 duration-500">
            ...
            <form onSubmit={handleCreateRoom} className="space-y-8">
              ...
              {/* ⚡ NO GUEST BLOCKER HERE: Let them simulate! ⚡ */}
              <button 
                type="submit"
        ...
        {view === "join" && (
          <div className="max-w-md mx-auto w-full bg-white/90 backdrop-blur-xl p-8 md:p-12 rounded-[2rem] border border-white shadow-2xl text-center animate-in slide-in-from-bottom-8 duration-500">
            ...
            <form onSubmit={handleJoinRoom} className="space-y-6">
              ...
              {/* ⚡ GUEST BLOCKER FOR JOINING FIREBASE ROOMS ⚡ */}
              ...
              {view === "waiting" && (
                <div className="absolute inset-0 z-10 flex flex-col p-8 md:p-12 text-center bg-transparent overflow-y-auto">
                  ...
                  <div className="mt-auto pt-4">
                    {isHost ? (
                      /* ⚡ GUEST BLOCKER FOR STARTING THE ACTUAL GAME ⚡ */
                      <GuestBlocker role="student">
                 ...
                 {isHost && (
                   <div className="relative mb-4">
                     <form onSubmit={handleSearchSong} className="flex gap-2">
                       ...
                       {/* ⚡ GUEST BLOCKER FOR MUSIC SEARCH ⚡ */}
                       <GuestBlocker role="student">
              ...
              <div className="bg-white/90 backdrop-blur-xl rounded-3xl border border-white shadow-xl flex-1 flex flex-col overflow-hidden min-h-[300px]">
                ...
                <div className="p-4 bg-white border-t border-slate-100">
                  ...
                  <form onSubmit={sendChatMessage} className="relative">
                    ...
                    {/* ⚡ GUEST BLOCKER FOR SENDING CHAT ⚡ */}
                    <GuestBlocker role="student">
